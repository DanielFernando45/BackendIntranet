  ...
  export class CommonService {
    ...
    async listarPorAsesoramiento(idAsesoramiento: number, stakeholder: UserRole) {
      ...
      if (stakeholder === UserRole.ASESOR) {
        asesorias = await this.procesosAsesoriaRepo.find({
          where: { asesor: { id: idAsesoramiento } },
          relations: ['asesoramiento'],
        });
      }

      if (asesorias.length === 0) return [];

      const idsAsesoramiento = asesorias.map((a) => a.asesoramiento.id);

      // 2. Buscar solo reuniones, contratos y asuntos
      const reuniones = await this.reunionRepo.find({
        where: { asesoramiento: { id: In(idsAsesoramiento) } },
        relations: ['asesoramiento'],
      });
