...
export class ModifyAndAddTables1755986930321 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`asesor\` CHANGE \`id_area\` \`id_supervisor\` int NULL`);
        ...
        await queryRunner.query(`CREATE TABLE \`rol\` (\`id\` varchar(255) NOT NULL, \`nombre\` enum ('admin', 'asesor', 'estudiante', 'jefe_operaciones', 'supervisor', 'contrato_pago', 'asesor_inducciones', 'soporte') NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`asesor\` DROP COLUMN \`id_supervisor\``);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`asesor\` DROP COLUMN \`id_supervisor\``);
        await queryRunner.query(`ALTER TABLE \`asesor\` ADD \`id_supervisor\` int NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`asesor\` CHANGE \`id_supervisor\` \`id_area\` int NULL`);
